define(['core/ajax', 'core/str'], function(Ajax, Str) {
    ...
    function statutVisuel(r) {
        var statut = r.status || 'idle';
        var label = 'Statut';
        var cls = 'idle';
        var base;
        var lastStatus = r.last_status || '';
        var isDerniere = false;
        var progress = r.progress || 0;
        var message = r.message || '';
        var error = r.error || '';
        if (statut === 'idle' && lastStatus) {
            statut = lastStatus;
            isDerniere = true;
        ...
        if (statut === 'queued') {
            base = 'Indexation en attente';
            label = isDerniere ? 'Dernière attente' : 'En attente';
            cls = 'queued';
        } else if (statut === 'running') {
            base = 'Indexation en cours : ' + progress + ' %';
            if (message) {
                base += ' - ' + message;
            }
            label = isDerniere ? 'Dernière en cours' : 'En cours';
            cls = 'running';
        } else if (statut === 'completed') {
            base = (isDerniere ? 'Dernière indexation terminée : ' : 'Indexation terminée : ') +
                (r.fichiers || 0) + ' PDF reçus, ' +
                (r.fichiersexploitables || 0) + ' exploitables, ' +
                (r.chunks || 0) + ' passages.';
            if (r.pagesocr) {
                base += ' OCR : ' + r.pagesocr + ' page' + (r.pagesocr > 1 ? 's' : '') +
                    ' dans ' + r.fichiersocr + ' PDF.';
            }
            label = 'Terminé';
            cls = 'completed';
        } else if (statut === 'failed') {
            base = (isDerniere ? 'Dernière indexation en erreur' : 'Indexation en erreur') +
                (error ? ' : ' + error : '.');
            label = 'Erreur';
            cls = 'failed';
        } else {
            base = 'Aucune indexation en cours.';
        }

        var maj = formaterDateCourte(
            r.updated_at || r.started_at || r.last_updated_at || r.last_started_at
        );
        var meta = '';
        if (maj && (statut !== 'idle' || isDerniere)) {
            meta = (isDerniere ? 'Dernière mise à jour : ' : 'Mise à jour : ') + maj;
        }
        return { label: label, cls: cls, text: base, meta: meta, status: statut };
    }

    function rendreStatutHTML(info) {
        var meta = info.meta ? '<div class="brc_index_meta">' + escHtml(info.meta) + '</div>' : '';
        return '<div class="brc_index_badge ' + escHtml(info.cls) + '">' + escHtml(info.label) + '</div>' +
            '<div class="brc_index_text">' + escHtml(info.text) + meta + '</div>';
    }

    function afficherEtatIndexation(r) {
        var info = statutVisuel(r);
        var zone = document.getElementById('brc-messages');
        hideWelcome();
        if (!indexStatusBubble || !document.body.contains(indexStatusBubble)) {
            indexStatusBubble = document.createElement('div');
            indexStatusBubble.className = 'brc_msg brc_msg_bot brc_index_status';
            indexStatusBubble.innerHTML = '<div class="brc_msg_row">' +
                '<div class="brc_av">' + SVG_BOT + '</div>' +
                '<div class="brc_bubble brc_index_bubble">' + rendreStatutHTML(info) + '</div>' +
                '</div>';
            zone.appendChild(indexStatusBubble);
        } else {
            var bubble = indexStatusBubble.querySelector('.brc_index_bubble');
            if (bubble) {
                bubble.className = 'brc_bubble brc_index_bubble ' + info.cls;
                bubble.innerHTML = rendreStatutHTML(info);
            }
        }
        scrollBas();
    }
    ...
    function suivreIndexation(courseid, afficherSiInactive, includeFinished) {
        ...
        .then(function(r) {
            ...
            if (r.status === 'queued' || r.status === 'running') {
                ...
                afficherEtatIndexation(r);
                indexPollTimer = window.setTimeout(function() {
                    suivreIndexation(courseid, true, true);
                }, 3000);
                return;
            }

            if (btn) btn.disabled = false;
            if (afficherSiInactive || r.last_status) {
                afficherEtatIndexation(r);
            }
        })
        .catch(function() {
    ...
    function reindexer(courseid) {
        var btn = document.getElementById('brc-reindex');
        if (btn) btn.disabled = true;
        afficherEtatIndexation({
            status: 'queued',
            progress: 0,
            message: 'Envoi des PDF au service d’indexation...'
        });
        ...
        .then(function(r) {
            afficherEtatIndexation({
                status: 'queued',
                progress: 0,
                message: r.fichiers + ' PDF reçus. Préparation de l’indexation...'
            });
            suivreIndexation(courseid, true, true);
        })
        .catch(function() {
            afficherEtatIndexation({
                status: 'failed',
                error: 'Le lancement a échoué. Vérifiez que le service IA est disponible.'
            });
            if (btn) btn.disabled = false;
        });
    }
    ...
    return {
        init: function(courseid, canreindex, coursename) {
            ...
            if (canreindex) {
                suivreIndexation(courseid, false, false);
            }
